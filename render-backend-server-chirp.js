...
function cleanTextForChirp(text) {
  let t = text
    ...
    .replace(/<[^>]+>/g, '')
    // Ranges WITH units first (most specific). \d+(?:\.\d+)? matches
    // decimals too (e.g. 0.25-3.0), which TTS reads as "zero point two five".
    .replace(/(\d+(?:\.\d+)?)\s*[\u002D\u2010\u2011\u2012\u2013\u2014\u2015\u2212]\s*(\d+(?:\.\d+)?)\s*(min(?:ute)?s?)\b/gi, '$1 to $2 minutes')
    .replace(/(\d+(?:\.\d+)?)\s*[\u002D\u2010\u2011\u2012\u2013\u2014\u2015\u2212]\s*(\d+(?:\.\d+)?)\s*(hr?s?|hours?)\b/gi, '$1 to $2 hours')
    .replace(/(\d+(?:\.\d+)?)\s*[\u002D\u2010\u2011\u2012\u2013\u2014\u2015\u2212]\s*(\d+(?:\.\d+)?)\s*(cm|mm|km|m)\b/gi, '$1 to $2 $3')
    .replace(/(\d+(?:\.\d+)?)\s*[\u002D\u2010\u2011\u2012\u2013\u2014\u2015\u2212]\s*(\d+(?:\.\d+)?)\s*°\s*C/gi, '$1 to $2 degrees Celsius')
    .replace(/(\d+(?:\.\d+)?)\s*[\u002D\u2010\u2011\u2012\u2013\u2014\u2015\u2212]\s*(\d+(?:\.\d+)?)\s*°\s*F/gi, '$1 to $2 degrees Fahrenheit')
